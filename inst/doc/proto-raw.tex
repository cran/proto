...
\Abstract{
  ...
  can be used in a class-based style as well.
  The key design goals of the package are to integrate into \proglang{R}
  while providing nothing more than a thin layer on top of it.
  This paper describes how these goals are achieved
  within \pkg{proto} and provides examples
  of the use of the \code{"proto"} package.
}
...
whether an object is a proto object.  In addition, \code{dot.proto}
...
\code{proto} passes the receiver object through the argument list,
while hiding this from the caller.  It defines the \code{proto} class
as a subclass of the \proglang{S3} \code{environment} class so that
...
One particularly important use of the \pkg{proto} package
...
\pkg{proto} package online help for complete syntax of commands):
...
with an \code{add} method and a method \code{init} which constructs
new objects.  In the line creating object \code{add1} the expression
\code{Add\$init(x = 1:5)} invokes the \code{init} constructor of the
receiver object \code{Add}. The method \code{init} has an argument of
...
\code{init} which is inherited by \code{Logadd} from \code{Add}
...
\code{proto} has no code that is specific to \code{ls}, \code{str} or
...
The \code{dot.proto} function can be used to create output that is
readable by the GraphViz \code{dot} program \citep{Ganser+North:1999}
to create visual representations of ancestor trees (figure
\ref{fig:proto-dot}).  It takes two arguments, a \code{proto} object
...
parent using \code{that\$proto} before the parent is even finished
being defined.  Compared to the cited \proglang{S4} example there is
...
are able to reference the yet to be defined parent via \code{that} as
\code{proto} makes \code{that} available to inserted components as
they are inserted.
...
the package \pkg{proto} 0.2--3. \proglang{R} itself and the

...
returns a function derived from \code{add} with the first argument,
the \code{proto} object,
already inserted.  Thus, if one wants exactly the original \code{add}
...
readable by the \proglang{GraphViz} \code{dot} program \citep{Ganser+North:1999}
to create visual representations of ancestor trees (figure
\ref{fig:proto-dot}).  It takes three arguments, a \code{proto} object
...
\code{file} argument denotes the name of the output file to hold
the generated \code{dot} code.  It defaults to output to the console
if it is omitted.  A third argument, \code{control}, 
also usually omitted, provides additional control over the output.  
In the following example we graph all \code{proto} objects in the
current environment placing the resulting \code{dot} code in
\code{test.dot}.
...
than premature abstractions through classes.  The \code{dot.proto}
